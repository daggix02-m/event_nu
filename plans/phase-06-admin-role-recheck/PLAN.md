...
- [ ] 1. `RequireAdmin(auth)`: parse the Bearer token for identity only → set
...
- [ ] 2. Leave the non-admin `RequireAuth` path claiming-trusted (no per-request
...
- [ ] 4. Run the gate; update `docs/PROGRESS.md`; commit on `main`.
...
- [ ] Admin authorization is re-verified server-side on every admin request.
- [ ] Demotion takes effect immediately (no wait for JWT expiry).
- [ ] Staleness window for non-admin endpoints documented.