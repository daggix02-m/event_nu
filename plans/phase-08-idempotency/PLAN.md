...
- [ ] 1. Migration `00010_idempotency_keys.sql`:
      ...
      `GRANT` to `app_user`.
- [ ] 2. `repository/idempotency.go`: `Create(ctx, userID, key, requestHash,
...
- [ ] 3. New middleware `Idempotency(app)`:
...
- [ ] Spec §22 semantics live on the three write endpoints (replay / reused-409).
- [ ] Decision log (`docs/idempotency.md`) lists protected vs safe endpoints.
- [ ] Retry cannot create a duplicate row for protected writes.