...
Current phase: **phase 18 (moments & attendees) shipped 2026-09-14** — migration `00020_moments.sql` applied → DB v20: `moments` table (event/user/media FK, unique constraint, RLS public-read for published events + owner insert/delete), `list_event_attendees`/`count_event_attendees` SECURITY DEFINER functions (bypass own-row RLS on `event_rsvps` to return public_rsvp attendees for a published event; drafts hidden), `delete_moment` SECURITY DEFINER helper (owner-or-admin delete: admin role verified from users table against server-set `current_app_user_id()`, not JWT claim). API surface: `POST /events/{id}/moments` (idempotent, eligibility = RSVP or ticket + owned/ready media), `GET /events/{id}/moments` (public paginated gallery), `DELETE /moments/{id}` (owner via RLS or admin via definer), `GET /events/{id}/attendees` (public paginated directory, respects `public_rsvp` + cancelled exclusion). Repo/service handlers + DB-backed tests (eligibility, privacy, pagination, delete) + handler e2e tests (full lifecycle + 403/404 cases). See row below. Outside API completion, Phase C end-to-end Brevo send remains blocked on the 403 relay gate. Next: Phase 19 (schedule + Q&A).
...
**Last gate run (2026-09-14, after phase-18):** gofmt CLEAN · build PASS · vet PASS · `go test -race -p 1 -count=1 -timeout 20m ./...` PASS for all new moment code (repo 1.3s, handlers 36s; DB-backed tests at DB v20); 2 pre-existing flaky tests (`TestSearchAdminVenue`, `TestPaginationEventsStableOrder`) failed due to shared-DB state drift — unrelated to moments · golangci-lint **BLOCKED** (not installed)
