...
Current phase: **phase 16 (offline sync) shipped 2026-09-11** — migration 00018 (DB v18), cursor-based `GET /api/v1/sync?cursor=&domains=&limit=` with inclusive `updated_at` delta across the public-discovery catalog, `op: upsert`/`op: delete` per row, `sync_deleted_ids()` SECURITY DEFINER for soft-delete visibility (RLS hides them from plain SELECTs), per-domain watermark over the full read horizon (`SyncPage`) so boundary re-emission never stalls pagination, monotonic `next_cursor`, `has_more` emitted only while pages remain, DTOs stripped of per-user decorations/media URLs (private domains full-refresh client-side). See `docs/sync-protocol.md` + row below. Outside API completion, Phase C end-to-end Brevo send remains blocked on the 403 relay gate. Next: Phase 17 (notifications push).
...
**Last gate run (2026-09-11, after phase-16):** gofmt CLEAN · build PASS · vet PASS · `go test -race -p 1 -count=1 -timeout 20m ./...` PASS (all 17 packages `ok`, handlers 683s; DB-backed tests ran against local `event_nu_test` at v18) · golangci-lint **BLOCKED** (not installed)
