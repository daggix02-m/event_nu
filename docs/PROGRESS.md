...
Current phase: **phase 19 (schedule & Q&A) shipped 2026-09-14** — migration `00021_schedule_qa.sql` applied → DB v21: `event_sessions` table (event FK CASCADE, title/speaker/stage, starts_at/ends_at, CHECK end≥start, index on event+starts_at; RLS: SELECT published+organizer+privileged, INSERT/UPDATE/DELETE organizer+privileged), `event_questions` table (event+user FK CASCADE, body CHECK 1..2000, pinned, answer; index on event+pinned; RLS: SELECT published+privileged, INSERT own, UPDATE own+organizer+privileged), `event_question_votes` table (PK question+user, created_at; RLS: SELECT true, INSERT/DELETE own+privileged). API surface: `POST /events/{id}/schedule` (idem, organizer), `PATCH /schedule/{id}` (organizer), `DELETE /schedule/{id}` (organizer), `GET /events/{id}/schedule` (public, ordered starts_at); `POST /events/{id}/questions` (signed-in), `GET /events/{id}/questions` (optional auth for voted_by_me, pinned first then vote-desc), `POST /questions/{id}/upvote` (idem), `DELETE /questions/{id}/upvote` (idem), `POST /questions/{id}/pin` (organizer), `POST /questions/{id}/answer` (organizer). Repo/service/handler tests pass. Next: Phase 20 (badges & recaps).
...
**Last gate run (2026-09-14, after phase-19):** gofmt CLEAN · build PASS · vet PASS · `go test -count=1 -timeout 10m ./internal/api/handlers/ -run 'TestSchedule|TestQA'` PASS (6 tests) · `go test -count=1 -timeout 10m ./internal/repository/ -run 'TestSchedule'` PASS (2 tests) · DB at v21 · 2 pre-existing flaky tests (shared-DB state drift) · golangci-lint **BLOCKED** (not installed)
