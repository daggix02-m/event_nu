...
Current phase: **phase 17 (push notifications FCM) shipped 2026-09-14** — migration 00019 (DB v19): `user_devices` registry (unique token, RLS own-row + privileged, `upsert_user_device` SECURITY DEFINER for cross-user token handover on re-login — returns the upserted row via `RETURN QUERY` since a SELECT can't read its own subquery's insert), dispatch columns on `notifications` (`dispatch_attempts`/`dispatched_at`/`next_dispatch_at`/`dispatch_lease_at`) + `notifications_dispatch_idx`, `event_reminders.dispatch_lease_at`. Push provider abstraction (`noop`/`fcm`) with a real FCM v1 HTTP client (service-account JWT → OAuth2 token via existing `golang-jwt`, no new deps), `POST /api/v1/devices` / `GET /api/v1/me/devices` / `DELETE /api/v1/devices/{id}`, worker push consumer (claim batch → send to all recipient devices → dispatched/backoff `base*2^min(attempt-1,6)`/attempt-cap; invalid token pruned; due reminders → `event_reminder` notification + `MarkReminderSent`). RSVP fan-out (`NotifyEventRSVPs` + `ListUserIDsByEvent`) plumbed but has **no trigger today** — comments are flat and only draft events are editable → documented deviation (`docs/push.md`). See `docs/push.md` + row below. Outside API completion, Phase C end-to-end Brevo send remains blocked on the 403 relay gate. Next: Phase 18 (moments/attendees).
...
**Last gate run (2026-09-14, after phase-17):** gofmt CLEAN · build PASS · vet PASS · `go test -race -p 1 -count=1 -timeout 20m ./...` PASS (all 21 packages `ok`, handlers 805s incl. new device e2e + dispatch tests; DB-backed tests ran against local `event_nu_test` at v19) · golangci-lint **BLOCKED** (not installed)
